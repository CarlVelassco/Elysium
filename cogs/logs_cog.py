...
class MakserSelect(discord.ui.Select):
    ...
    def __init__(self, cog_instance):
        ...
        try:
            ...
            # В случае ошибки оставляем options пустым, чтобы сработала логика ниже
            options = []

        # Если после всех манипуляций список опций пуст (например, ошибка чтения файла)
        # показываем заглушку.
        if not options:
        ...
        super().__init__(
            ...
            max_values=1, 
            options=options,
            # Отключаем меню, только если единственная опция - это заглушка "disabled"
        ...
        async for message in channel.history(limit=None, after=start_time, before=end_time):
            ...
            for embed in message.embeds:
                ...
                if embed.description:
                    ...
                    for i, part in enumerate(desc_parts):
                        if part.startswith('<@') and part.endswith('>'):
                            try:
                                user_id_str = part.replace('<@', '').replace('>', '')
                                data['user_id'] = int(user_id_str)
                                nick = ' '.join(desc_parts[i+1:]).replace('`', '').strip()
                ...
                for field in embed.fields:
                    ...
                    print(f"   -> Поле: '{field.name}' | Значение: '{field.value}' | Очищенное имя: '{clean_field_name}'")

                    if clean_field_name == 'получено':
                        try:
                            points_str = field.value.replace('`', '').split()[0]
                            data['points'] = int(points_str)
                        except (ValueError, IndexError):
